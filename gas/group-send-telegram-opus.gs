function demoSendOpusToTelegramGroup() {
  ...
  sendOpusToTelegramGroup(group_name, group_admin, opusBase64, filename, caption);
}


function sendOpusToTelegramGroup(group_name, group_admin, opusBase64, filename, caption) {
  ...
  Logger.log("Calling API to send OPUS to this group " + group_name);
  UrlFetchApp.fetch("https://api.whatsmate.net/v3/telegram/group/voice_note/message/" + instanceId, options);
}
